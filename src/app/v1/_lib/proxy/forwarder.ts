...
export class ProxyForwarder {
  ...
  private static async doForward(
    ...
    // 将 Claude 的 /v1/messages 或 Response API 的 /v1/responses 路径重写为 OpenAI 标准端点
    if (
      toFormat === "openai-compatible" &&
      (fromFormat === "claude" || fromFormat === "codex" || fromFormat === "gemini-cli")
    ) {
      forwardUrl = new URL(session.requestUrl);
      ...
      if (originalPath === "/v1/messages/count_tokens") {
        ...
        // 标准端点（/v1/messages 或 /v1/responses）
        forwardUrl.pathname = "/v1/chat/completions";
